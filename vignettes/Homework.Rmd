...
summary(lm.D1)$coef
```
**The $R^2$ is `r summary(lm.D1)$r.squared` **
```{r}
par(mar=c(1,1,1,1))
par(mfrow=c(2,2))
plot(lm.D1)
...
$$
*According to the inverse transformation method，we can generate random samples from a Rayleigh(σ) distribution.In the first example,take σ=1,we can find that the mode of the generated samples is close to the theoretical mode σ(check the histogram).*
...
*Continue, we take σ=5 and σ=10 respectively，the empirical distribution of the sample is still close to the true distribution *
### σ=5 ###
```{r}
n <- 1000
set.seed(1234)
...
*Write a function to generate a random sample from a $W_{d}(\Sigma, n)$(Wishart) distribution for n > d + 1 ≥ 1, based on Bartlett’s decomposition.We take n=6,d=4,and$$Σ=
\left[\begin{array}{cccc}{1} & {0.7} & {0.3} & {0.9} \\ {0.7} & {1} & {0.2} & {0.6} \\ {0.3} & {0.2} & {1} & {0.5} \\ {0.9} & {0.6} & {0.5} & {1}\end{array}\right]
$$*
...
*We can't say the powers are different at 0.05 level.*
  ...
  * *The corresponding hypothesis test problem is whether the powers are different, that is "$H_{0}: pwr_{1}=pwr_{2}$ vs $H_{1}: pwr_{1} \neq pwr_{2}$".*
  * *We should use paired-t test.*
  * *There is only one sample for each of $pwr_{1}$ and $pwr_{2}$ in the title, so multiple experiments should be performed to generate a plurality of $pwr_{1}$ and $pwr_{2}$, thereby obtaining a plurality of differences of $pwr_{1}$ and $pwr_{2}$. It is noted that the difference between $pwr_{1}$ and $pwr_{2}$  is d, a plurality of d approximately obey the t distribution. The null hypothesis can be tested by applying paired-t test.*
...
to display the scatter plots for each pair of test scores. Compare the plot with the sample correlation matrix. Obtain bootstrap estimates of the standard errors for each of the following estimates:$$
\begin{array}{l}{\hat{\rho}_{12}=\hat{\rho}(\mathrm{mec}, \mathrm{vec}), \hat{\rho}_{34}=\hat{\rho}(\mathrm{alg},} \ { \text { ana } ), \hat{\rho}_{35}=\hat{\rho}(\mathrm{alg}, \mathrm{sta}), \hat{\rho}_{45}=\hat{\rho}(\mathrm{ana}, \mathrm{sta})}\end{array}
$$
...
* Power comparison (distance correlation test versus ball covariance test)
    ...
    * Model1: Y =X/4+e
    * Model2: Y =X/4×e
    * X ∼ $N\left(0_{2}, l_{2}\right)$, e ∼ $N\left(0_{2}, l_{2}\right)$, X and e are independent.
...
*Comparing the figure and the table, it can be seen that when the standard deviation is taken as 0.5 and 5, the overall effect is better.Moreover,the latter is relatively better than the former *
## Question
